...
function showContextMenu(event) {
  ...
  const submenuAnalysis = document.getElementById("submenuAnalysis");
  submenuAnalysis.innerHTML = ""; // Réinitialiser le sous-menu
  ...
  if (isThematicMode) {
    for (let key in properties) {
      if (properties.hasOwnProperty(key)) {
        // Ne pas ajouter les clés indésirables (commençant ou finissant par "fit" ou "forme", ou contenant "id")
        if (
          key.startsWith("fit") ||
          key.endsWith("fit") ||
          key.startsWith("forme") ||
          key.endsWith("forme") ||
        ...
        if (typeof properties[key] !== "number" || properties[key] == null) {
          continue;
        }

        // Créer un élément de sous-menu pour chaque propriété numérique
        const menuItemAnal = document.createElement("div");
        menuItemAnal.className = "submenu-item";
        menuItemAnal.textContent = capitalizeFirstLetter(key);

        // Ajouter un événement clic pour mettre à jour le label
        menuItemAnal.onclick = function () {
          hideContextMenu();
          console.log("la cle selectionee est ...", key);
          UpdateAnalysis({
            Currentgeojson: currentGeoJson,
            propertyToAnalyze: key,
           
          });
        };

        submenuAnalysis.appendChild(menuItemAnal);
      }
    }
  }
}
...
function toggleLabel(layer) {
  if (layer.getTooltip()) {
    ...
    const labelText =
      ...
      layer.feature.properties.divison ||
...
document.getElementById("AllLabel").addEventListener("click", function () {
  if (currentLayer) {
    try {
      ...
      layerBoundsGroupAnalysis.eachLayer(function (layer) {
        toggleLabelsForLayer(layer);
      });
    } catch (error) {}
  }
...
function insertMarker(latlng) {
  if (latlng) {
    ...
    console.error(
      "Impossible d'ajouter un marqueur. Coordonnées non définies."
    );
  }
